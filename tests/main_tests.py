...
def test_parseResults():
    ...
    if len(testDict) != len(expectedDict):
        pytest.fail("Test failed.")

    for test_title, test_hsp in testDict.iteritems():
        if not test_title in expectedDict.keys():
            pytest.fail(str(test_title) +  "\nis not in the expected dictionary. Test failed.")
            break
        hash_hsp = hashlib.md5()
        hash_hsp.update(str(test_hsp))
        if expectedDict[test_title] != str(hash_hsp.hexdigest()):
            pytest.fail("The HSP \n" + str(test_hsp) + "\nisn't associated with the right key. Test failed." )
